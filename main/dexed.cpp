    ...
    for (uint8_t note = 0; note < max_notes; ++note) {
      if (voices[note].live) {
        ...
        for (uint8_t j = 0; j < _N_; ++j) {
          ...
          // ARM SSAT instruction:
          // variable is shifted by 9.
          // then saturate to the negative of the desired value.
          // and saturate to the positive of the desired value.
          
           int32_t clip_val;
           val = val >> 9;
           // This is for ESP32!
           asm volatile("clamps a8, a9, 15" : "=r" (clip_val)
                                           : "r"  (val)
                                            );
